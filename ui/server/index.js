...
async function setupProjectsWatcher() {
  ...
  const geminiProjectsPath = path.join(process.env.HOME, '.qwen', 'projects');
    ...
    try {
      ...
      if (data.type === 'init') {
        ...
        const welcomeMsg = hasSession ?
          `\x1b[36mResuming Dekacode session ${sessionId} in: ${projectPath}\x1b[0m\r\n` :
          `\x1b[36mStarting new Dekacode session in: ${projectPath}\x1b[0m\r\n`;
        ...
        try {
          // Get dekacode command from environment or use default
          const geminiPath = process.env.DEKACODE_PATH || 'dekacode';

          // First check if dekacode CLI is available
          try {
            execSync(`which ${geminiPath}`, { stdio: 'ignore' });
          } catch (error) {
            // console.error('❌ Dekacode CLI not found in PATH or DEKACODE_PATH');
            ws.send(JSON.stringify({
              type: 'output',
              data: `\r\n\x1b[31mError: Dekacode CLI not found. Please check:\x1b[0m\r\n\x1b[33m1. Install dekacode binary in PATH\x1b[0m\r\n\x1b[33m2. Or set DEKACODE_PATH in .env file\x1b[0m\r\n`
            }));
            return;
          }
          
          // Build shell command that changes to project directory first, then runs dekacode
