...
async function spawnGemini(command, options = {}, ws) {
  return new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      // Check for MCP config in ~/.qwen.json
      const geminiConfigPath = path.join(os.homedir(), '.qwen.json');
      ...
      if (fsSync.existsSync(geminiConfigPath)) {
        try {
          ...
          if (!hasMcpServers && geminiConfig.qwenProjects) {
            const currentProjectPath = process.cwd();
            const projectConfig = geminiConfig.qwenProjects[currentProjectPath];
      ...
      if (hasMcpServers) {
        ...
        if (fsSync.existsSync(geminiConfigPath)) {
          try {
            ...
            const currentProjectPath = process.cwd();
            const projectConfig = geminiConfig.qwenProjects && geminiConfig.qwenProjects[currentProjectPath];
    ...
    const modelToUse = options.model || 'qwen-2.5-72b-instruct';
    // Debug - Using model
    ...
    // Try to find dekacode in PATH first, then fall back to environment variable
    const dekacodeePath = process.env.DEKACODE_PATH || 'dekacode';
    // console.log('Full command:', dekacodeePath, args.join(' '));

    const geminiProcess = spawn(dekacodeePath, args, {
      cwd: workingDir,
    ...
    // Removed complex response handler to preserve text formatting
    ...
    geminiProcess.stdout.on('data', (data) => {
      ...
      console.log('Chunk length:', rawOutput.length);
      console.log('Chunk (hex):', Buffer.from(rawOutput).toString('hex'));
      console.log('Chunk (escaped):', JSON.stringify(rawOutput));
      console.log('Chunk (raw):', rawOutput);
      ...
      // Don't process individual chunks - wait for process to complete
      // This prevents mangling of streaming data
    });
    ...
    geminiProcess.on('close', async (code) => {
      ...
      // Process the complete output buffer now that process is finished
      if (outputBuffer.trim()) {
        console.log('=== FINAL BUFFER ===');
        console.log('Buffer length:', outputBuffer.length);
        console.log('Buffer (hex):', Buffer.from(outputBuffer).toString('hex'));
        console.log('Buffer (escaped):', JSON.stringify(outputBuffer));
        console.log('Buffer (raw):', outputBuffer);
        console.log('=== END BUFFER ===');

        // Temporarily disable filtering to debug text chunking issues
        const filteredOutput = outputBuffer.trim();

        console.log('=== FILTERED OUTPUT ===');
        console.log('Filtered length:', filteredOutput.length);
        console.log('Filtered (escaped):', JSON.stringify(filteredOutput));
        console.log('Filtered (raw):', filteredOutput);
        console.log('=== END FILTERED ===');

        if (filteredOutput) {
          // Set the full response for session storage
          fullResponse = filteredOutput;

          // For new sessions, create a session ID
          if (!sessionId && !sessionCreatedSent && !capturedSessionId) {
            capturedSessionId = `dekacode_${Date.now()}`;
          ...
          // Send the complete, properly formatted response as a single message
